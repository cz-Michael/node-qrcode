...
QRCodeDraw.prototype = {
  ...
  draw:function(canvas,text,options,cb){
    ...
    if (options.color && options.color.dark) {
      this.color.dark = options.color.dark
    }

    if (options.color && options.color.light) {
      this.color.light = options.color.light
    }

    if (options.marginScaleFactor) {
      this.marginScaleFactor = options.marginScaleFactor
    }
